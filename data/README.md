...
- `versions` (object): keys are exact Minecraft versions (minor updates matter).
  - Each value is an array of builds for that Minecraft version.
    ...
    - `sourceUrl` (string, optional): link to the commit or release/tag page (e.g., `https://github.com/JsMacros/JsMacros/commit/<sha>` or `https://github.com/<owner>/<repo>/releases/tag/<tag>`).
